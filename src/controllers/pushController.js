...
//푸시 알림 핸들러
function notificationHandler(kstMinute, kstHour, message) {
  console.log(`스케줄링 시작`);
...
const sendPushNotice = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const job = schedule.scheduleJob(
      `0 ${kstMinute} ${kstHour} * * *`,
      function () {
        notificationHandler(kstMinute, kstHour, message);
      }
    );
...
const updateAlarmTime = asyncHandler(async (req, res) => {
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    return res
      .status(400)
      .json({ message: "유효하지 않은 시간 또는 분입니다." });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "알람 시간과 기기 토큰이 저장되었습니다.", alarm });
  } catch (error) {
    console.error("알람 시간 및 기기 토큰 저장 중 오류 발생:", error);
    res
      .status(500)
      .json({ message: "알람 시간 및 기기 토큰 저장에 실패했습니다.", error });
  }
});
